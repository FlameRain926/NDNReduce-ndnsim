...
Interest::wireEncode(EncodingImpl<TAG>& encoder) const
{
  ...
  // MachineRank
  if (getRank() != 65535){
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::MachineRank, machine_rank);
  }
...
Interest::wireDecode(const Block& wire)
{
  ...
  for (++element; element != m_wire.elements_end(); ++element) {
    switch (element->type()) {
      ...
      case tlv::MachineRank: {
        if (lastElement >= 8) {
          break; // MachineRank is non-critical, ignore out-of-order appearance
        }
        if (element->value_size() != 1) {
          NDN_THROW(Error("MachineRank element is malformed"));
        }
        machine_rank = readNonNegativeInteger(*element);
        lastElement = 8;
        break;
      }
